...
const fetchresult = async () => {
    try {
      const response = await fetch('/overview/Filterresults/'+ searchstring); // Adjust your API endpoint accordingly
      if (response.ok) {
              ...
              <div class="container flex justify-between mx-auto">
                  <div class="w-full lg:w-8/12">
                      <div class="flex items-center justify-between">
                          <h1 class="text-xl font-bold text-[#173e26] md:text-2xl"> Results For:{searchstring}</h1>
                          <div>
                          <select
                          ...
                          <div className="flex items-center justify-between">
                            <span className="font-light text-gray-600">
                              {new Date(result.created).toLocaleDateString('en-US', {
                                ...
                                <div className="mt-2"><Link to="/" className="text-2xl font-bold text-[#173e26] hover:underline">{result.pdfs.title}</Link></div>
